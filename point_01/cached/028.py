...
class SpatioTemporalGraphConv(nn.Module):
    def __init__(self, in_features: int, out_features: int, num_joints: int,
        ...
        nn.init.xavier_uniform_(self.temporal_adj.data, gain=1.414)

        # 构建固定的空间邻接矩阵（人体结构先验）
        self.spatial_adj = self._build_spatial_adjacency()  # 不再使用register_buffer

        # 时间关系的mask（只允许相同关节在不同时间步之间连接）
        self.temporal_mask = self._build_temporal_mask()  # 不再使用register_buffer
    ...
    def _build_spatial_adjacency(self):
        ...
        connections = [
            # 躯干到四肢
            (0, 1), (0, 4), (0, 7), (0, 11),  # Neck -> 四肢根部
            # 左臂
            (1, 2), (2, 3),  # LeftArm -> LeftForeArm -> LeftHand
            # 右臂
            (4, 5), (5, 6),  # RightArm -> RightForeArm -> RightHand
            # 左腿
            (7, 8), (8, 9), (9, 10),  # LeftUpLeg -> LeftLeg -> LeftFoot -> LeftToeBase
            # 右腿
            (11, 12), (12, 13), (13, 14)  # RightUpLeg -> RightLeg -> RightFoot -> RightToeBase
        ]
    ...
    def _build_temporal_mask(self):
        """构建时间关系mask，只允许相同关节在不同时间步连接"""
        mask = torch.zeros(self.time_steps - 1, self.num_joints, self.num_joints)
        for t in range(self.time_steps - 1):
            for j in range(self.num_joints):
                mask[t, j, j] = 1.0  # 只允许相同关节在不同时间步连接
        return mask

    def _construct_spatiotemporal_adjacency(self, device):
        """构建时空邻接矩阵，确保在正确的设备上"""
        # 将组件移动到目标设备
        spatial_adj = self.spatial_adj.to(device)
        temporal_mask = self.temporal_mask.to(device)
        temporal_adj = self.temporal_adj

        # 应用mask到可学习的时间关系参数
        masked_temporal_adj = temporal_adj * temporal_mask
        ...
        for t_i in range(self.time_steps):
            for t_j in range(self.time_steps):
                ...
                if t_i == t_j:
                    ...
                    full_adj[row_start:row_end, col_start:col_end] = masked_temporal_adj[time_idx]
...
# 简化的PositionalEncoding类（修复可能的错误）
...
class EnhancedSpatioTemporalTransformer(nn.Module):
    def __init__(
        ...
        # 使用新的时空图卷积层
        if use_graph_conv:
            self.graph_conv = SpatioTemporalGraphConv(3, 3, num_keypoints, time_steps)

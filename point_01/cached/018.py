...
class LearnableTemporalConv(nn.Module):
    """可学习时间卷积层，建模关节点间的时间关系"""

    def __init__(self, in_features: int, out_features: int, temporal_window: int = 3, bias: bool = True):
        super().__init__()
        ...
        self.temporal_conv = nn.Conv1d(
            ...
            padding=(temporal_window - 1) // 2,  # 保持时间维度不变
            bias=bias
        )

        # 可学习的时间注意力权重，增强重要时间步
        self.time_attention = nn.Parameter(torch.ones(1, 1, 1, dtype=torch.float))

    def forward(self, input: Tensor) -> Tensor:
        B, T, V, J, C = input.shape

        # 重塑为时间卷积格式 (B*V*J, C, T)
        x = input.permute(0, 3, 2, 1, 4).contiguous()  # (B, J, V, T, C)
        x = x.view(B * J * V, T, C).permute(0, 2, 1)  # (B*J*V, C, T)

        # 应用时间卷积
        x_temp = self.temporal_conv(x)  # (B*J*V, out_features, T)

        # 应用时间注意力
        x_temp = x_temp * F.softmax(self.time_attention, dim=-1)

        # 重塑回原始格式
        x_temp = x_temp.permute(0, 2, 1)  # (B*J*V, T, out_features)
        x_temp = x_temp.view(B, J, V, T, self.out_features)
        x_temp = x_temp.permute(0, 3, 2, 1, 4)  # (B, T, V, J, out_features)
...
class EnhancedSpatioTemporalTransformer(nn.Module):
    ...
    def __init__(
            ...
            use_temporal_conv: bool = True,  # 新增：是否使用时序卷积
            temporal_window: int = 3,  # 时序卷积窗口大小
    ):
        super().__init__()
        self.embed_dim = embed_dim
        ...
        if use_temporal_conv:
            self.temporal_conv = LearnableTemporalConv(3, 3, temporal_window)
    ...
    def forward(self, joints_3D: Tensor) -> Tensor:
        ...
        # 并行应用空间和时间建模
        if self.use_graph_conv and self.use_temporal_conv:
            # 同时使用图卷积和时序卷积
            graph_output = self.graph_conv(joints_3D)
            graph_output = self.graph_norm(graph_output)

            temporal_output = self.temporal_conv(joints_3D)
            temporal_output = self.temporal_norm(temporal_output)

            # 残差连接融合
            joints_3D = (original_joints +
                         self.graph_res_weight * graph_output +
                         self.temporal_res_weight * temporal_output)

        elif self.use_graph_conv:
            # 仅使用图卷积
            graph_output = self.graph_conv(joints_3D)
            graph_output = self.graph_norm(graph_output)
            joints_3D = original_joints + self.graph_res_weight * graph_output

        elif self.use_temporal_conv:
            # 仅使用时序卷积
            temporal_output = self.temporal_conv(joints_3D)
            temporal_output = self.temporal_norm(temporal_output)
            joints_3D = original_joints + self.temporal_res_weight * temporal_output

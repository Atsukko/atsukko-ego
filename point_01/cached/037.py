...
class SpatioTemporalTransformer(nn.Module):
    def __init__(
        ...
        # ===== 辅助专家（全部 zero-mean）=====
        self.aux1 = nn.Conv1d(
            in_dim, in_dim, 3, padding=2, dilation=2, groups=in_dim, bias=False
        )
        self.aux2 = nn.AvgPool1d(3, stride=1, padding=1)
        ...
        # 限制 aux 最大影响
        self.aux_scale = nn.Parameter(torch.tensor(0.03))
    ...
    def forward(self, joints_3D: Tensor, *args, **kwargs) -> Tensor:
        ...
        # ===== Temporal MoE =====
        x_t = joints_3D_fl_flat.transpose(1, 2)  # (B, C, T)

        main_out = self.main(x_t)
        ...
        # 去 DC 分量（关键！）
        aux1 = aux1 - aux1.mean(dim=-1, keepdim=True)
        aux2 = aux2 - aux2.mean(dim=-1, keepdim=True)

        w = self.gate(joints_3D_fl_flat.mean(dim=1))  # (B, 2)
        ...
        out = main_out + self.aux_scale * aux

        joints_3D_fl_flat = joints_3D_fl_flat + out.transpose(1, 2)

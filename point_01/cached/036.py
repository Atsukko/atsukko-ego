...
class MotionAwareTemporalRefiner(nn.Module):
    """
    创新点：基于运动感知的时序优化 (Motion-Aware Temporal Refiner)
    原理：利用一阶差分（速度）来动态决定是否需要时序平滑。
    - 快速运动时：减少平滑，保留真实动作细节。
    - 慢速/静止时：增加平滑，消除抖动。
    """
    ...
    def __init__(self, in_dim, kernel_size=3):
        ...
        # 2. 运动感知门控 (Motion Gate)
        # 输入是速度特征，输出是 0~1 的权重
        self.motion_gate = nn.Sequential(
            nn.Conv1d(in_dim, in_dim // 4, kernel_size=1),  # 降维
            nn.Tanh(),  # Tanh 允许负值，保留方向性特征
            nn.Conv1d(in_dim // 4, in_dim, kernel_size=1),  # 升维
            nn.Sigmoid()  # 输出 0-1 的门控值
        )

        # 初始化：让 gate 初始值接近 0，保证初始状态接近 identity mapping
        # 这样模型会从基线开始学习，避免一开始就破坏精度
        nn.init.constant_(self.motion_gate[-2].weight, 0.0)
        nn.init.constant_(self.motion_gate[-2].bias, 2.0)

    def forward(self, x):
        # x shape: (B, T, C) -> (B, C, T)
        x_in = x.transpose(1, 2)

        # 计算时序差分 (近似速度 Velocity)
        # padding 保证时间维度一致
        # diff[t] = x[t] - x[t-1]
        padded_x = F.pad(x_in, (1, 0), mode='replicate')
        velocity = padded_x[:, :, 1:] - padded_x[:, :, :-1]

        # 计算平滑后的特征
        x_smooth = self.smoother(x_in)

        # 计算门控权重：网络根据速度自动决定“混合比例”
        # gate 越接近 1，表示越多地采用平滑特征
        gate = self.motion_gate(velocity)

        # 动态残差连接
        # Output = Original + Gate * (Smoothed - Original)
        # 等价于: (1 - Gate) * Original + Gate * Smoothed
        out = x_in + gate * (x_smooth - x_in)
...
class SpatioTemporalTransformer(nn.Module):
    def __init__(
        ...
        self.temporal_mix = MotionAwareTemporalRefiner(in_dim)
    ...
    def forward(self, joints_3D: Tensor, *args, **kwargs) -> Tensor:
        ...
        # ===== Temporal residual mixing =====
        x_tm = self.temporal_mix(joints_3D_fl_flat)
        joints_3D_fl_flat = joints_3D_fl_flat + x_tm

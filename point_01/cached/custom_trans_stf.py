...
from framevision.model.components import CustomTransformerEncoder
from framevision.model.components import CustomTransformerEncoderLayer
...
def build_local_temporal_mask(
    ...
    """
    Returns:
        attn_mask: (T, T), float tensor with 0 or -inf
    """
    mask = torch.full((T, T), float("-inf"), device=device, dtype=dtype)

    for t in range(T):
        left = max(0, t - window_size)
        right = min(T, t + window_size + 1)
        mask[t, left:right] = 0.0
...
class SpatioTemporalTransformer(nn.Module):
    def __init__(
        ...
        encoder_layer = CustomTransformerEncoderLayer(d_model=embed_dim, nhead=num_heads, dropout=dropout, batch_first=True)
    ...
    def forward(self, joints_3D: Tensor, *args, **kwargs) -> Tensor:
        ...
        if self.temporal_window is not None:
            attn_mask = build_local_temporal_mask(
                T=x.size(1),
        ...
        x = self.transformer_encoder(x, mask=attn_mask)  # Shape: (B, T, embed_dim)

...
class LearnableGraphConv(nn.Module):
    def __init__(self, in_features: int, out_features: int, num_joints: int, bias: bool = True):
        ...
        # 可学习的稀疏化参数
        self.sparsity_threshold = nn.Parameter(torch.tensor(0.005))  # 阈值
        self.sparsity_strength = nn.Parameter(torch.tensor(1.0))  # 强度控制

        # 限制参数范围，确保稳定性
        self.sparsity_threshold.data.clamp_(min=0.0, max=0.05)
        self.sparsity_strength.data.clamp_(min=0.5, max=3.0)
    ...
    def forward(self, input: Tensor) -> Tensor:
        ...
        # 轻微稀疏化
        # 使用sigmoid确保平滑过渡，避免突变
        sparse_mask = torch.sigmoid((adj - self.sparsity_threshold) * self.sparsity_strength)  # 缩放因子使过渡更平滑
        adj = adj * sparse_mask

...
class SELayer(nn.Module):
    """ Squeeze-and-Excitation Layer: 自适应调整通道重要性 """
...
class MultiScaleTemporalRefiner(nn.Module):
    ...
    def __init__(self, in_dim):
        ...
        # 为了保持轻量化，我们使用 groups=in_dim (Depthwise Convolution)
        # 分支1: 捕捉局部高频运动 (Kernel 3)
        self.branch_small = nn.Sequential(
            nn.Conv1d(in_dim, in_dim, kernel_size=3, padding=1, groups=in_dim, bias=False),
            nn.BatchNorm1d(in_dim),
            nn.ReLU(inplace=True)
        )

        # 分支2: 捕捉中长时序运动 (Kernel 5)
        # 也可以用 dilation=2 的 kernel=3 来增大感受野
        ...
        # 特征融合层 (Pointwise Conv)
        # 将两个分支的信息融合，这一步会增加参数量，但对提升精度很有帮助
        self.fusion = nn.Conv1d(in_dim * 2, in_dim, kernel_size=1, bias=False)

        # 注意力机制：动态加权
        self.se = SELayer(in_dim)

        # 最终的LayerNorm (可选，用于稳定训练)
        self.ln = nn.LayerNorm(in_dim)

    def forward(self, x):
        # x shape: (B, T, C) -> 需要转换为 (B, C, T) 进行 Conv1d
        x_in = x.transpose(1, 2)

        out_small = self.branch_small(x_in)
        out_large = self.branch_large(x_in)

        # 拼接并融合
        out = torch.cat([out_small, out_large], dim=1)  # (B, 2C, T)
        out = self.fusion(out)
...
class SpatioTemporalTransformer(nn.Module):
    def __init__(
        ...
        # 替换掉了原本简单的 Conv1d
        # 使用新的多尺度时序精炼模块
        self.temporal_mix = MultiScaleTemporalRefiner(in_dim)
        # ================= 修改结束 =================
    ...
    def forward(self, joints_3D: Tensor, *args, **kwargs) -> Tensor:
        ...
        # ================= 修改开始 =================
        # 调用新的模块
        # 注意：这里不需要再手动 transpose 了，因为我在 MultiScaleTemporalRefiner 内部处理了
        # 输入: (B, T, C), 输出: (B, T, C)
        joints_3D_fl_flat = self.temporal_mix(joints_3D_fl_flat)
        # ================= 修改结束 =================

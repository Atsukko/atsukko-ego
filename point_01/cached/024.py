...
class LearnableGraphConv(nn.Module):
    def __init__(self, in_features: int, out_features: int, num_joints: int, bias: bool = True):
        ...
        # 新增：可学习的温度参数（初始为0，对应温度1.0，通过exp确保正）
        self.log_temperature = nn.Parameter(torch.zeros(1))  # 标量参数
    ...
    def forward(self, input: Tensor) -> Tensor:
        ...
        # 新增：应用可学习温度控制softmax锐度
        temperature = torch.exp(self.log_temperature)  # 确保温度为正
        adj = F.softmax(adj / temperature, dim=-1)  # 自适应归一化

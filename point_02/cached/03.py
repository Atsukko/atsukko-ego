...
class ViewJointReliability(nn.Module):
    """
    Predicts reliability weights for each (view, joint) observation.
    """
    def __init__(self, num_joints, num_views, hidden_dim=32):
        super().__init__()
    ...
    def forward(self, joints):
        """
        joints: (B, T, V, J, 3)
        return: reliability weights in (0,1), shape (B, T, V, J, 1)
        """
        B, T, V, J, C = joints.shape
        x = joints.view(B * T * V * J, C)

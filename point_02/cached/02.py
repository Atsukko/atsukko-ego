...
class SpatioTemporalTransformer(nn.Module):
    def __init__(
        ...
        self.temporal_mix = nn.Conv1d(
            in_channels=num_views * num_keypoints * 3,
            out_channels=num_views * num_keypoints * 3,
            kernel_size=3,
            padding=1,
            groups=num_views * num_keypoints * 3,  # 每个 (V,J,C) 独立
            bias=False
        )
        ...
        self.temporal_reliability = nn.Sequential(
            nn.Conv1d(
                ...
                kernel_size=3,
                padding=1,
                groups=num_views * num_keypoints * 3,
                bias=False
            ),
            nn.Sigmoid()
        )
    ...
    def forward(self, joints_3D: Tensor, *args, **kwargs) -> Tensor:
        ...
        # (B, V, J, C, T)
        x = joints_3D.permute(0, 2, 3, 4, 1).contiguous()
        ...
        # low-light-aware reliability (0~1)
        rel = self.temporal_reliability(x)

        # reliability-guided temporal refinement
        x = x + rel * x_tm

        # reshape 回来
        x = x.view(B, V, J, C, T)
        joints_3D = x.permute(0, 4, 1, 2, 3).contiguous()  # (B, T, V, J, 3)
